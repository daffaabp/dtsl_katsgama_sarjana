			...
			<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
				...
				<thead>
					...
					<tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
						<th class="min-w-125px">Propinsi</th>
						...
						<tr>
							...
							<td class="text-end">
								<div class="d-flex justify-content-end flex-shrink-0">
									<a @click.prevent="handleEdit(<?= $pr['id'] ?>)" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
									...
									<a @click.prevent="handleDelete(<?= $pr['id'] ?>)" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
			...
			<div class="modal-content">
				<form id="add_form" @submit.prevent="handleAddPost" autocomplete="off">
					...
					<div class="modal-body">
						<div>
							<div class="fv-row">
								<label class="col-form-label required fw-semibold fs-6">Nama Propinsi</label>
								<input type="text" x-model="formData.nama" name="nama" class="form-control form-control-lg form-control-solid" placeholder="" value="" />
							</div>
						</div>
					</div>
				...
				<form id="edit_form" @submit.prevent="handleEditPost" autocomplete="off">
					...
					<div class="modal-body">
						<div>
							<div class="fv-row">
								<label class="col-form-label required fw-semibold fs-6">Nama Propinsi</label>
								<input type="text" x-model="formData.nama" name="nama" required class="form-control form-control-lg form-control-solid" placeholder="" value="" />
	...
	document.addEventListener('alpine:init', () => {
		var rules = {
			fields: {
				'nama': {
					validators: {
						notEmpty: {
							message: 'Nama Propinsi Harus diisi'
						}
					}
				},
			},
		...
		Alpine.data('ui', () => ({
			...
			async handleAddPost(e) {
				...
				if (status == 'Valid') {
					try {
						...
						let req = await fetch(site_url + 'propinsi/add', {
							headers: {
			...
			async handleEdit(id) {
				...
				try {
					this.loading = true;
					let req = await fetch(site_url + 'propinsi/edit/' + id);
					this.loading = false;
					if (req.ok) {
						let res = await req.json();
						this.formData = res;
			...
			async handleEditPost(id) {
				...
				if (status == 'Valid') {
					try {
						let req = await fetch(site_url + 'propinsi/edit/' + this.formData.id, {
							headers: {
								'Content-Type': 'application/json'
							},
							method: 'POST',
			...
			async handleDelete(id) {
				...
				if (sw.isConfirmed) {
					let req = await fetch(site_url + 'propinsi/delete/' + id);
					if (req.ok) {
						let rt = await Swal.fire({
